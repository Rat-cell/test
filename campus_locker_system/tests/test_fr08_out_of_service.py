...
class TestFR08OutOfService:
    ...
    def test_fr08_admin_set_locker_out_of_service(self, app):
        ...
        with app.app_context():
            ...
            admin, admin_msg = AdminAuthService.create_admin_user(
                ...
                role="ADMIN" # Assuming AdminRole.ADMIN or similar if defined
            )
    ...
    def test_fr08_audit_trail_locker_status_changes(self, app):
        ...
        with app.app_context():
            ...
            # Check audit log using AuditLogRepository
            # Assuming get_logs can filter. If not, get_paginated_logs and filter in Python.
            # For simplicity, let's assume AuditLogRepository.get_logs() can take keyword arguments for filtering
            # or a more specific method exists. If not, this will need adjustment.
            # audit_logs = AuditLog.query.filter_by(action="ADMIN_LOCKER_STATUS_CHANGED").all() # Original
            audit_logs_page = AuditLogRepository.get_paginated_logs(page=1, per_page=AuditLogRepository.get_count()) # Get all logs
            audit_logs = [log for log in audit_logs_page.items if log.action == "ADMIN_LOCKER_STATUS_CHANGED"]

            assert len(audit_logs) > 0, "FR-08: Should create audit log entry for ADMIN_LOCKER_STATUS_CHANGED"
            
            # Find the specific log for this test; there might be others from previous tests if DB is not perfectly clean
            # or if other actions also log this. Best to filter by details if possible or ensure a clean state.
            # For now, we take the latest one, assuming it's the one we just created.
            latest_log = None
            for log_entry in reversed(audit_logs): # Check newest first
                details = json.loads(log_entry.details) if log_entry.details else {}
                if details.get('locker_id') == locker.id and details.get('admin_id') == admin.id:
                    latest_log = log_entry
                    break
            
            assert latest_log is not None, "FR-08: Could not find the specific audit log for this status change."
