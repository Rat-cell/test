...
from app.business.admin import AdminRole
...
class TestFR07AuditTrail:
    ...
    def test_fr07_deposit_audit_events(self, app, test_lockers):
        ...
        with app.app_context():
            ...
            assert "parcel_id" in latest_deposit_log.details, "Should log parcel ID"
            assert "locker_id" in latest_deposit_log.details, "Should log locker ID"
            assert "recipient_email" in latest_deposit_log.details, "Should log recipient email"
            
            print(f"   ✅ Deposit audit log created: {latest_deposit_log.action}")
            print(f"   ✅ Timestamp recorded: {latest_deposit_log.timestamp}")
            print(f"   ✅ Parcel ID logged: {latest_deposit_log.details.get('parcel_id')}")
            print(f"   ✅ Locker ID logged: {latest_deposit_log.details.get('locker_id')}")
            ...
            with patch('app.config.get', side_effect=lambda key, default=None: 
                      False if key == 'ENABLE_EMAIL_BASED_PIN_GENERATION' else (default or False)):
                with app.test_request_context():
    ...
    def test_fr07_admin_action_audit_events(self, app, test_lockers, test_admin):
        ...
        with app.app_context():
            ...
            with app.test_request_context():
                locker_result, status_message = update_locker_status(
